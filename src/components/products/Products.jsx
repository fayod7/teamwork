    ...
    <section className='w-full py-14 bg-[#f0f0f0]'>
      <div className="container">
        ...
        <div className="grid grid-cols-4 gap-6  max-[1150px]:grid-cols-3 max-[950px]:gap-4 max-[750px]:grid-cols-2 max-[450px]:grid-cols-1">
           {
            data?.products?.map(({id, thumbnail, title, price, rating, description})=>(
              <div style={{
...
}} key={id} className='flex flex-col gap-3 bg-white rounded-lg overflow-hidden py-5 px-4'>
                <div className='overflow-hidden flex justify-center items-center h-48 w-full relative'>
                  <div className="absolute top-2 right-2 bg-white rounded-full p-2 cursor-pointer shadow-md" onClick={toggleLike}>
                        {
                          like? 
                  ...
                  <img onClick={()=> navigate(`/product/${id}`)} className='duration-200 hover:cursor-pointer hover:scale-90 object-contain h-full' src={thumbnail} alt="" />
                </div>
         ...
         <button className='text-[#B88E2F] border border-[#B88E2F] py-3 px-16 block mx-auto mt-5 bg-white duration-200 hover:cursor-pointer hover:bg-[#B88E2F] hover:text-white'>See more</button>
      </div>
    </section>
  )
}
